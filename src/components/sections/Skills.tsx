...
const Skills = () => {
	...
	return (
		<section
			id="skills"
			className="py-20 border-t border-gray-900 min-h-[80vh] flex flex-col justify-center"
		>
			<motion.div
				initial={{ opacity: 0 }}
				...
				<div className="flex flex-col md:flex-row gap-8 bg-[#0a0a0a] border border-gray-800 rounded-xl p-4 md:p-10 relative overflow-hidden">
					...
					<div className="absolute inset-0 grid grid-cols-[repeat(20,minmax(0,1fr))] opacity-5 pointer-events-none">
						{[...Array(400)].map((_, i) => (
							<div
								key={i}
								className="border-[0.5px] border-primary/20 aspect-square"
							/>
						))}
					</div>
							...
							>
								...
								{activeCategory === cat.id && (
									<motion.div
										...
										className="ml-auto w-1.5 h-1.5 rounded-full bg-primary animate-pulse"
									/>
								)}
							</button>
						))}
